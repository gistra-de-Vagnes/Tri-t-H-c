...
class Command(BaseCommand):
    help = 'Initialize the database with sample philosophy data'

    def handle(self, *args, **options):
        self.stdout.write('Creating sample philosophy schools...')
        ...
        try:
            ...
            self.stdout.write("Sample user 'philosopher' already exists")
        except User.DoesNotExist:
            user = User.objects.create_user(
                username="philosopher",
                password="philosopher123"
            )
            self.stdout.write("Created sample user: philosopher")
            ...
            if created:
                self.stdout.write("Created profile for sample user")
